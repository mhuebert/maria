...
(deftest dependencies-test
  (cell/reset-namespace 'cells.cell-test)
...
(deftest cell-function
  ...
  (let [p (cell (mapv cell (repeat 1) [:c :d]))]
    (is (= (mapv deref @p) [:c :c])
...
(deftest contexts
  ...
  (let [ctx-1 (eval-context/new-context)
        ctx-2 (eval-context/new-context)]
